#123700, 123072, 122787, 116847, 100036
...
class ThreeAddressCodeGenerator(Equation):
    ...
    def generate(self):
        ...
        while (i < n):
            ...
            if (super().is_operator(e)):
                ...
                if e not in super().operators1:
                    ...
                    if (track == 1):
                        global label2
                        label2 = Label(root, text=ch + ":=" + top_ele + e + exp[i + 1], font=("times", 10), fg="white",
                                       bg="#4e3620")
                        label2.pack(pady=10)
                        ...
                        global label3
                        label3 = Label(root, text=ch + ":=" + prev_val + e + exp[i + 1], font=("times", 10), fg="white",
                                       bg="#4e3620")
                        label3.pack(pady=10)
        ...
        while (tracker > -1):
            ...
            if (track1 > 1):
                global label4
                label4 = Label(root, text=ch + ":=" + operand1 + operator + prev_val, font=("times", 10), fg="white",
                               bg="#4e3620")
                label4.pack(pady=10)
                ...
                global label5
                label5 = Label(root, text=ch + ":=" + operand1 + operator + operand2, font=("times", 10), fg="white",
                               bg="#4e3620")
                label5.pack(pady=10)
...
root = Tk()
root.title("Three Address code generator app")
...
label = Label(root, text="start with 'anyword'= then input the arithmetic expression in any order\n use only one letter eg(s) not (oo)", font=("times", 10), fg="white", bg="#4e3620")
label.pack(pady=5)

label = Label(root, text="Don't use spaces in between letters", font=("times", 10), fg="white", bg="#4e3620")
label.pack(pady=5)
